...
import { getPrimitivePrefabUuid } from '../internal-assets';
import { getPageArgs, paginate } from '../paging';
import * as fs from 'fs';
import * as path from 'path';
import { v4 as uuidv4 } from 'uuid';

// Built-in effect UUIDs (from library/.internal-info.json, engine 3.8.8)
// effects/builtin-standard.effect → c8f66d17-351a-48da-a12c-0212d28575c4
// effects/builtin-unlit.effect    → a3cd009f-0ab0-420d-9278-b9fdab939bbc
// effects/builtin-toon.effect     → 9b20a514-6cc3-49de-b216-b6b863046249
// Built-in effect UUIDs (final storage format, verified against imported .mtl files)
 ...
 */
function buildMaterialJson(name: string, effectUuid: string, color: { r: number; g: number; b: number; a: number }): string {
...
export class NodeTools implements ToolExecutor {
    ...
    getTools(): ToolDefinition[] {
        return [
            ...
            {
                name: 'get_all_nodes',
                description: 'Get all nodes in the scene with their UUIDs (paginated)',
                inputSchema: {
                    type: 'object',
                    properties: {
                        page_size: {
            ...
            {
                name: 'create_primitive_node',
                description: 'Create a 3D primitive mesh node (box, cylinder, cone, sphere, plane, capsule, torus, quad). Instantiates the engine\'s built-in prefab (same as right-click "Create → 3D Object") — the prefab already includes MeshRenderer, correct mesh, and default material. When "color" is provided, automatically creates a material asset (.mtl) with that color and assigns it to the primitive. One call, no manual assembly.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        type: {
                            ...
                            enum: ['box', 'cylinder', 'cone', 'sphere', 'plane', 'capsule', 'torus', 'quad']
                        },
                        name: {
                            type: 'string',
                            description: 'Node name'
                        ...
                        parentUuid: {
                            type: 'string',
                            description: 'Parent node UUID (uses scene root if omitted)'
                        },
                        position: {
                            type: 'object',
                        ...
                        color: {
                            type: 'object',
                            description: 'RGBA color {r, g, b, a} (0-255). When provided, auto-creates a material asset (.mtl) in assets/materials/ and assigns it to the primitive\'s MeshRenderer.',
                            properties: {
                                r: { type: 'number', default: 200 },
                                g: { type: 'number', default: 200 },
                        ...
                        effect: {
                            ...
                            default: 'standard',
                            description: 'Shader effect for the auto-created material (only used when color is provided)'
                        }
                    },
                    required: ['type', 'name']
                }
    ...
    async execute(toolName: string, args: any): Promise<ToolResponse> {
        switch (toolName) {
            ...
            case 'find_nodes':
                const fnResult = await this.findNodes(args.pattern, args.exactMatch);
                if (fnResult.success && Array.isArray(fnResult.data)) {
                    const { pageSize: ps, cursor: c } = getPageArgs(args);
                    fnResult.data = paginate(fnResult.data, ps, c);
                }
                return fnResult;
            case 'find_node_by_name':
                return await this.findNodeByName(args.name);
            case 'get_all_nodes':
                const gaResult = await this.getAllNodes();
            ...
            case 'create_primitive_node':
                return await this.createPrimitiveNode(args);
            default:
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (args.assetPath && !finalAssetUuid) {
                    try {
                        const assetInfo = await AssetDB.queryAssetInfo(args.assetPath);
    ...
    // ── Built-in 3D Primitive Prefab Instantiation ───────────────────────
    // Uses Cocos Creator's engine-internal prefabs (the same ones the editor
    // uses for right-click "Create → 3D Object"). Paths resolved to UUIDs at
    // startup via internal-assets.ts, which reads library/.internal-info.json.
    // This replaces the old approach of manually creating an empty node +
    // adding MeshRenderer + set-property for mesh — one IPC call instead of 4.

    private async createPrimitiveNode(args: any): Promise<ToolResponse> {
        try {
            const prefabUuid = getPrimitivePrefabUuid(args.type);
            if (!prefabUuid) {
                return {
                    success: false,
                    error: `Unknown primitive type: '${args.type}'. Supported: box, sphere, cylinder, cone, plane, quad, capsule, torus`
                };
            }
            ...
            if (!parentUuid) {
                try {
                    ...
                    parentUuid = sceneInfo?.uuid || null;
                } catch (err) {
                    console.warn('[NodeTools] Failed to get scene root UUID for primitive');
                }
            }

            // Instantiate the built-in prefab (one call — prefab already has
            // MeshRenderer + correct mesh + default material)
            ...
            // ── Auto-create material when color is specified ──────────
            let materialPath = '';
            let materialUuid = '';
            if (args.color) {
                try {
                    const effect = args.effect || 'standard';
                    const effectUuid = BUILTIN_EFFECT_UUIDS[effect] || BUILTIN_EFFECT_UUIDS['standard'];
                    const safeName = (args.name || 'primitive').replace(/[^a-zA-Z0-9_-]/g, '_');
                    const mtlName = `Mat_${safeName}`;
                    materialPath = `db://assets/materials/${mtlName}.mtl`;
                    materialUuid = uuidv4();

                    // Write .mtl file
                    const mtlContent = buildMaterialJson(mtlName, effectUuid, args.color);
                    const relPath = materialPath.replace('db://', '');
                    const absPath = path.join(Editor.Project.path, relPath);
                    const dir = path.dirname(absPath);
                    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
                    fs.writeFileSync(absPath, mtlContent, 'utf-8');

                    // Use saveAssetMeta to write .meta with importer="material"
                    // (direct fs write gets overwritten by editor with importer="*")
                    await AssetDB.saveAssetMeta(materialPath, JSON.stringify({
                        ver: '1.0.21',
                        importer: 'material',
                        imported: true,
                        uuid: materialUuid,
                    ...
                    // Assign material via scene script (most reliable path)
                    await new Promise(r => setTimeout(r, 300));
                    Editor.Message.request('scene', 'execute-scene-script', {
                        name: 'cocos-mcp-server',
                        method: 'assignMaterialToRenderer',
                        args: [nodeUuid, materialUuid, 0],
                    }).catch(() => {});
                } catch (err) {
                    console.warn('[NodeTools] Material auto-creation failed:', err);
                }
            }
            ...
            return {
                ...
                data: {
                    ...
                    materialUuid: materialUuid || undefined,
                    message: `Primitive '${args.name}' (${args.type}) created from built-in prefab` +
                        (materialUuid ? ` with material ${materialPath}` : '')
                }
            };
        } catch (err: any) {
            return {
                success: false,
                error: `Failed to create primitive node: ${err.message}`
            };
        }
    }
}