...
export class DebugTools implements ToolExecutor {
    ...
    private async getPerformanceStats(): Promise<ToolResponse> {
        try {
            ...
            const nodeCount = tree?.children ? this.countNodes(tree.children) : (Array.isArray(tree) ? this.countNodes(tree) : 0);
            return { success: true, data: { nodeCount, componentCount: 0, drawCalls: 0, triangles: 0, message: `Scene has ${nodeCount} nodes` } };
        } catch {
