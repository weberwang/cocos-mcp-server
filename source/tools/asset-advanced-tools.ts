...
export class AssetAdvancedTools implements ToolExecutor {
    ...
    private async saveAssetMeta(urlOrUUID: string, content: string): Promise<ToolResponse> {
        return new Promise((resolve) => {
            AssetDB.saveAssetMeta(urlOrUUID, content).then((result: any) => {
                resolve({
                    success: true,
                    data: {
                        uuid: result?.uuid,
    ...
    private async generateAvailableUrl(url: string): Promise<ToolResponse> {
        return new Promise((resolve) => {
            AssetDB.generateAvailableUrl(url).then((availableUrl: string) => {
                resolve({
                    success: true,
                    data: {
                        originalUrl: url,
    ...
    private async queryAssetDbReady(): Promise<ToolResponse> {
        return new Promise((resolve) => {
            AssetDB.queryReady().then((ready: boolean) => {
                resolve({
                    success: true,
                    data: {
                        ready: ready,
    ...
    private async openAssetExternal(urlOrUUID: string): Promise<ToolResponse> {
        return new Promise((resolve) => {
            AssetDB.openAssetExternal(urlOrUUID).then(() => {
                resolve({
                    success: true,
    ...
    private async validateAssetReferences(directory: string = 'db://assets'): Promise<ToolResponse> {
        return new Promise(async (resolve) => {
            try {
                ...
                const assets = await AssetDB.queryAssets(`${directory}/**/*`);
                ...
                for (const asset of assets) {
                    try {
                        const assetInfo = await AssetDB.queryAssetInfo(asset.url);
                        if (assetInfo) {
