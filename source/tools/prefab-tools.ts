...
export class PrefabTools implements ToolExecutor {
    ...
    async execute(toolName: string, args: any): Promise<ToolResponse> {
        switch (toolName) {
            case 'get_prefab_list':
                const plResult = await this.getPrefabList(args.folder);
                if (plResult.success && Array.isArray(plResult.data)) {
                    plResult.data = paginate(plResult.data, getPageArgs(args).pageSize, getPageArgs(args).cursor);
                }
                return plResult;
            case 'load_prefab':
    ...
    private async loadPrefab(prefabPath: string): Promise<ToolResponse> {
        try {
            const assetInfo = await AssetDB.queryAssetInfo(prefabPath);
            if (!assetInfo) return { success: false, error: 'Prefab not found' };
            // Try to read the actual prefab content
            const fsPath = AssetDB.dbUrlToFsPath(prefabPath);
            const fs = require('fs');
            if (fs.existsSync(fsPath)) {
                const content = fs.readFileSync(fsPath, 'utf-8');
                return { success: true, data: { uuid: assetInfo.uuid, name: assetInfo.name, content, message: 'Prefab loaded' } };
            }
            return { success: false, error: 'Prefab file not found on disk' };
        } catch (err: any) { return { success: false, error: err.message }; }
    }
    ...
    private async instantiatePrefab(args: any): Promise<ToolResponse> {
        return new Promise(async (resolve) => {
            try {
                // 获取预制体资源信息
                const assetInfo = await AssetDB.queryAssetInfo(args.prefabPath);
                if (!assetInfo) {
    ...
    private async readPrefabFile(prefabPath: string): Promise<any> {
        try {
            ...
            try {
                assetContent = await AssetDB.queryAssetInfo(prefabPath);
    ...
    private async tryCreateNodeWithPrefab(args: any): Promise<ToolResponse> {
        return new Promise((resolve) => {
            AssetDB.queryAssetInfo(args.prefabPath).then((assetInfo: any) => {
                if (!assetInfo) {
                    throw new Error('预制体未找到');
                }
    ...
    private async updatePrefab(prefabPath: string, nodeUuid: string): Promise<ToolResponse> {
        return new Promise((resolve) => {
            AssetDB.queryAssetInfo(prefabPath).then((assetInfo: any) => {
                if (!assetInfo) {
    ...
    private async createMetaWithAssetDB(assetPath: string, metaContent: any): Promise<{ success: boolean; data?: any; error?: string }> {
        return new Promise((resolve) => {
            ...
            AssetDB.saveAssetMeta(assetPath, metaContentString).then((assetInfo: any) => {
    ...
    private async reimportAssetWithAssetDB(assetPath: string): Promise<{ success: boolean; data?: any; error?: string }> {
        return new Promise((resolve) => {
            AssetDB.reimportAsset(assetPath).then((result: any) => {
