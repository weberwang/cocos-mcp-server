...
export const methods: { [key: string]: (...any: any) => any } = {
    ...
    /**
     * @en Write MCP config to OpenCode config file
     * @zh 将 MCP 配置写入 OpenCode 配置文件
     * @param location 'local' = project root, 'global' = ~/.config/opencode/
     */
    async configureOpenCode(location: string) {
        const fs = require('fs');
        const path = require('path');

        // Resolve absolute path to the stdio adapter
        const adapterPath = path.resolve(__dirname, '..', 'stdio-adapter', 'build', 'index.js');
        if (!fs.existsSync(adapterPath)) {
            return {
                success: false,
                error: `找不到 stdio 适配器: ${adapterPath}\n请先执行 npm run build 编译。`
            };
        }
        ...
        if (fs.existsSync(configPath)) {
            try {
                ...
                // Keep existing $schema if present
                if (!config.$schema) {
                    config.$schema = 'https://opencode.ai/config.json';
                }
            } catch (e) {
                console.warn('[MCP插件] 无法解析现有配置文件，将创建新文件:', e);
            }
        }
        ...
        console.log(`[MCP插件] 配置已写入: ${configPath}`);
